...
const MovieInfo = () => {
  ...
  // ⭐️ [수정] 최상위 div에서 absolute 속성 완전 제거
  return (
    <div className="text-white w-full"> {/* absolute, inset-0, z-10 등 제거 */}
      ...
      <div className="relative flex justify-center items-center">
        <div key={currentIndex} className="flex justify-around p-4 gap-8 fade-in w-full">
          <div className="flex-1 flex flex-col justify-between p-6 bg-gradient-to-tr from-[#1a1a1a] to-black bg-opacity-40 rounded-xl max-w-4xl mx-auto border border-gray-800 shadow-2xl shadow-black/50">
            <div className="flex flex-col md:flex-row gap-8"> {/* 모바일 화면을 위해 flex-direction 변경 */}
              <img
                // ⭐️ [수정] 높이 계산을 위한 ref 제거 (불필요)
                src={currentMovie.image}
                alt={currentMovie.title + " 포스터"}
                className="w-full md:w-1/2 object-contain self-center md:self-start rounded-lg shadow-lg"
              />
              {/* ⭐️ [수정] JS로 높이를 강제하는 style 제거 (불필요) */}
        ...
        {/* PREV/NEXT 버튼 위치 조정 (필요시 left/right 값 수정) */}
